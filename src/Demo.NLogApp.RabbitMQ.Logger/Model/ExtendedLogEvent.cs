...
namespace Demo.NLogApp.RabbitMQ.Logger.Model
{
    ...
    public class ExtendedLogEvent : ILogger, IReadOnlyList<KeyValuePair<string, object>>
    {
        ...
        public ExtendedLogEvent(string format, params object[] values)
        {
            _format = format;
            _parameters = values;
        }

        public ExtendedLogEvent AddProp(string name, object value)
        {
        ...
        public static Func<ExtendedLogEvent, Exception, string> Formatter { get; } = (l, e) => l.LogValues.ToString();
    }
